...
class WP_Exception extends Exception {
	...
	public function __construct( $code = '', $message = '', $data = '' ) {
		...
		if( $this->wp_error instanceof WP_Error ){
			$messages = array_map(
				function( $a, $b ){ return $a.': '.$b; },
				$this->wp_error->get_error_codes(),
	 ...
	 * returns an array of error strings on success, 	 
	 * or empty array on failure (if using code parameter)	 
	 */	
	 ...
	 * Returns mixed or null, if no errors.	 
	 */	
	...
	function add( $code = '', $message ='', $data = '' ){
		...
		if( empty( $code ) || !is_scalar( $code ) )
			return false;
		
		if( $this->wp_error instanceof WP_Error )
			$this->wp_error->add( $code, $message, $data );
		
		// Create WP Error if not exists		
		else			
	...
	/**	 
	 * Add data for error code. The error code can only contain one error data.	 
	 * No return.	 
	 */	
	...
	/**	 
	 * Remove any messages and data associated with an error code.	 
	 * No return.	 
	 */	
...
function is_wp_exception( $exception ){	
	return $exception instanceof WP_Exception;
}