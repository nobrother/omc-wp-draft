...
class OMC_Settings_Sanitizer {
	...
	function __construct() {
		...
		do_action_ref_array( 'omc_settings_sanitizer_init', array( &$this ) );
	 ...
	 *
	 * Array can be filtered via 'omc_available_sanitizer_filters' to let
	...
	function get_available_filters() {
		...
		return apply_filters( 'omc_available_sanitizer_filters', $default_filters );
 ...
 *
 * Use the 'omc_settings_sanitizer_init' action to be notified when this function is safe to use
 *
 * @since 1.7.0
 *
 * @uses OMC_Settings_Sanitizer::add_filter()
 *
 * @param string $filter The filter to call (see OMC_Settings_Sanitizer::$available_filters for options)
 ...
 * @return true
 */
function omc_add_option_filter( $filter, $option, $suboption = null ) {

	return OMC_Settings_Sanitizer::$instance->add_filter( $filter, $option, $suboption );

}

add_action( 'admin_init', 'omc_settings_sanitizer_init' );
/**
 * Instantiate the Sanitizer.
 *
 * @since 1.7.0
 */
function omc_settings_sanitizer_init() {

	new OMC_Settings_Sanitizer;
