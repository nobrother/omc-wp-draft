...
(function($){
	$(function(){
		...
		$('textarea.code-editor').each(function(){
			...
			html += '</select>';
			html += '</div>';
			html += '<div class="setting-font-size">';
...
// Ajax save
(function($, global, dom){
	var target = global.ctrl_s_target && global.ctrl_s_target.target || false,
			overlay = global.overlay || {};
	
	$(function(){
		target && $(target).on('submit', function(e){
			var $this = $(this);
			...
			$(global).off('beforeunload'); // Prevent the alert
			global.onbeforeunload = null; // Prevent the alert
		});
		...
		$(dom).on('keydown', '*', function(e){
			...
			if(e.ctrlKey && e.which === 83){
				e.preventDefault();
				target && $(this).closest('form').submit();
			}
		});
	})
